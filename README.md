...
1. Clone the repo
1. In the "backend" folder there is a file `example.env`.
...
#### Developers-only
1. Run `pip install -r backend/requirements/dev.txt`.
...
`backend/requirements/base.in` and `backend/requirements/dev.in` - This is where you would add python libraries that you want to include in the project.

`backend/requirements/base.txt` and `backend/requirements/dev.txt` - These files should NOT be manually edited. These files are auto generated by a script that takes in the `.in` file and outputs a `.txt` file with the proper versions of each library.
