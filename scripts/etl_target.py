...
def target_find():
    ...
    study_description_records = connect_and_execute_psql(
        "AACT",
        "SELECT bs.nct_id, CONCAT(bs.description, ' ',  dd.description) FROM ctgov.brief_summaries bs INNER JOIN ctgov.detailed_descriptions dd ON bs.nct_id = dd.nct_id LIMIT 50",
        None,
    )
