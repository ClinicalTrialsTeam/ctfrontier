...
def function_invoke():
    ...
    cmd = (
        ...
        f"--payload '{__base64_encode_dict(payload)}' "
...
def __deploy_docker_image():
    ...
    r = subprocess.run(cmd.split())
    if r.returncode != 0:
        click.secho("Error pushing docker image", fg="red")
        raise click.Abort()
...
def __update_function_code():
    ...
    cmd = (
        ...
        f"--image-uri {__image_uri()} --publish"
    )
